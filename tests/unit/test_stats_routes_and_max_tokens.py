...
class TestStatsWithPlugin:
    """有 DKI Plugin 时从 plugin.get_stats() 读取真实数据"""

    def test_total_requests_from_plugin(self):
        """totalRequests 应来自 plugin.get_stats()"""
        app = make_app(with_plugin=True)
        client = TestClient(app)
        data = client.get("/api/stats").json()
        assert data["dkiStats"]["totalRequests"] == 10

    def test_avg_alpha_from_plugin(self):
        """avgAlpha 应来自 plugin.get_stats()"""
        app = make_app(with_plugin=True)
        client = TestClient(app)
        data = client.get("/api/stats").json()
        assert abs(data["dkiStats"]["avgAlpha"] - 0.35) < 0.001

    def test_injection_rate_from_plugin(self):
        """injectionRate 应来自 plugin.get_stats()"""
        app = make_app(with_plugin=True)
        client = TestClient(app)
        data = client.get("/api/stats").json()
        assert abs(data["dkiStats"]["injectionRate"] - 0.7) < 0.001

    def test_cache_stats_from_plugin(self):
        """cacheStats 应来自 plugin.get_cache_stats()"""
        app = make_app(with_plugin=True)
        client = TestClient(app)
        data = client.get("/api/stats").json()
        cache = data["cacheStats"]
        assert cache["l1Size"] == 5
        assert cache["l1MaxSize"] == 1000

    def test_l1_l2_l3_distribution_from_logs(self):
        """L1/L2/L3 分布应从 injection_logs 推算"""
        app = make_app(with_plugin=True)
        client = TestClient(app)
        data = client.get("/api/stats").json()
        dki = data["dkiStats"]
        # mock logs: 1 L1, 1 L2, 1 none(L3)
        assert dki["l1Hits"] == 1
        assert dki["l2Hits"] == 1
        assert dki["l3Computes"] == 1

    def test_dki_sub_endpoint_with_plugin(self):
        """/api/stats/dki 也应从 plugin 读取"""
        app = make_app(with_plugin=True)
        client = TestClient(app)
        data = client.get("/api/stats/dki").json()
        assert data["totalRequests"] == 10

    def test_cache_sub_endpoint_with_plugin(self):
        """/api/stats/cache 也应从 plugin 读取"""
        app = make_app(with_plugin=True)
        client = TestClient(app)
        data = client.get("/api/stats/cache").json()
        assert data["l1Size"] == 5

    def test_plugin_exception_falls_back_to_counter(self):
        """plugin.get_stats() 抛异常时应回退到计数器"""
        app = FastAPI()
        app.include_router(create_stats_router())
        mock_plugin = MagicMock()
        mock_plugin.get_stats.side_effect = RuntimeError("plugin error")
        app.state.dki_plugin = mock_plugin
