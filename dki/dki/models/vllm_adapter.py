...
class VLLMAdapter(BaseModelAdapter):
    ...
    def _forward_with_flash_attention(
        ...
        **kwargs
        ...
        Note: Currently uses HuggingFace generate() with past_key_values,
        same as the standard path. KVInjectionOptimizer integration is
        planned for a future version when custom attention hooks are supported.
        """
        mem_len = injected_kv[0].key.shape[2] if injected_kv else 0
