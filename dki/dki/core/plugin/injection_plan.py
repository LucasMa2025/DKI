...
class QueryContext:
    ...
    P1-1: recall_token_budget 提供 token 级别的软预算控制，
    比条数限制更精确地控制 prompt 长度。
    """
    recall_limit: int = 10
    recall_token_budget: int = 2048     # P1-1: token 软预算
...
class ExecutionResult:
    ...
    kv_bytes_gpu_peak: int = 0             # 推理期间 GPU KV 峰值字节数
    kv_transfer_latency_ms: float = 0.0    # CPU→GPU 传输耗时 (ms)
    kv_layers_count: int = 0               # KV 层数