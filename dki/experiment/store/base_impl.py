"""
BaseChatStore - 共享 CRUD 实现

从 demo/store/base_impl.py 复制, 修改导入路径。

Author: AGI Demo Project
Version: 1.0.0 (forked from demo/store/base_impl.py 3.0.0)
"""
...
from dki.experiment.store.base import IChatStore, StoreOperationError, StoreNotConnectedError
from dki.experiment.store.bm25_mixin import BM25Mixin
from dki.experiment.store.connection import ExperimentDBManager, ExperimentDBConfig
...
class BaseChatStore(IChatStore, BM25Mixin):
    ...
    def __init__(self, config: ExperimentDBConfig):
        self.config = config
        self.db_manager = ExperimentDBManager(config)
        self.__init_bm25__()
