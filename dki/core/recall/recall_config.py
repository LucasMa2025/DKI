...
class RecallSignalsConfig:
    ...
    # P1: BM25 全文检索 (补充关键词信号)
    bm25_enabled: bool = True
    bm25_top_k: int = 15           # BM25 返回候选数
...
class RecallScoreWeights:
    ...
    keyword_weight: float = 0.3    # w1: 关键词权重 (jieba TF-IDF)
    bm25_weight: float = 0.2       # w4: BM25 全文检索权重 (P1)
    vector_weight: float = 0.3     # w2: 向量相似度权重
    recency_weight: float = 0.2    # w3: 时间近度权重
...
class EpistemicModeConfig:
    ...
    profiles: Dict[str, Dict[str, Any]] = field(default_factory=lambda: {
        "clarification": {
            "keyword_weight": 0.35, "bm25_weight": 0.20, "vector_weight": 0.10,
            "recency_weight": 0.35, "description": "用户在回忆或澄清过去的信息",
        },
        "continuation": {
            "keyword_weight": 0.15, "bm25_weight": 0.10, "vector_weight": 0.25,
            "recency_weight": 0.50, "description": "用户在延展当前话题",
        },
        "semantic_search": {
            "keyword_weight": 0.10, "bm25_weight": 0.15, "vector_weight": 0.50,
            "recency_weight": 0.25, "description": "用户在进行语义相关的检索",
        },
        "correction": {
            "keyword_weight": 0.10, "bm25_weight": 0.10, "vector_weight": 0.10,
            "recency_weight": 0.70, "description": "用户在修正之前的信息",
        },
        "direct_lookup": {
            "keyword_weight": 0.40, "bm25_weight": 0.25, "vector_weight": 0.20,
...
class RecallResult:
    ...
    keyword_hits: int = 0
    bm25_hits: int = 0          # P1: BM25 全文检索命中数
    vector_hits: int = 0
