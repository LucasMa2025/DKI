...
class InjectionExecutor:
    ...
    @staticmethod
    def _build_preference_prefix(preference_text: str, alpha: float) -> str:
        """
        构造偏好前缀文本, 用于 prompt_prefix 注入模式
        
        将格式化后的偏好文本包装为系统级指令前缀, 拼接在 prompt 前面,
        由 vLLM 在 prefill 阶段自然处理。
        
        Alpha 映射:
        - alpha >= 0.7: "严格遵循" (strong adherence)
        - alpha >= 0.4: "适当参考" (moderate reference)
        - alpha > 0.1:  "轻微参考" (light reference)
        
        Args:
            preference_text: 格式化后的偏好文本 (由 Planner._format_preferences 生成)
            alpha: 偏好强度 (0.0 ~ 1.0)
        
        Returns:
            带有系统指令包装的偏好前缀字符串, 末尾包含换行分隔符
        """
